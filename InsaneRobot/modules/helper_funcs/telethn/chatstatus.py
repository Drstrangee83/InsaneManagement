...
from InsaneRobot import DRAGONS
from InsaneRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Insane_is_admin(chat_id: int):
    status = False
    Insane = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Insane.id == user.id:
            status = True
            break
    return status
